...
exports.node = exports.builder = void 0;
...
const devNode_1 = __importDefault(require("./devNode"));
...
const computeArgv = (params, ignoreCmdLineArgs = false) => {
    ...
    return (0, exports.builder)((0, yargs_1.default)(cmdLineArgv))
...
const spawn = async (params) => {
    ...
    const spawnEndedPromise = new Promise((ok) => {
        anvil.on("close", ok);
    });
...
const deploy = async (params) => {
    ...
    const devNode = new devNode_1.default(params.provider);
    ...
    try {
        ...
        throw new Error("Could not get chain id, is the anvil node running?\nOriginal error: \n" +
            err.toString());
    }
    ...
    ${params.script}`;
        ...
        const scriptPromise = new Promise((ok, ko) => {
            childProcess.exec(forgeScriptCmd, {
                encoding: "utf8",
                env: env,
                cwd: CORE_DIR,
            }, (error, stdout, stderr) => {
    ...
    return {
        ...
        revert: async (snapshotId = lastSnapshotId) => {
            ...
            if (blockNumberAfterRevert != snapshotBlockNumber) {
                throw Error(`evm_revert did not revert to expected block number ${snapshotBlockNumber} but to ${blockNumberAfterRevert}. Snapshots are deleted when reverting - did you take a new snapshot after the last revert?`);
            }
        },
    };
};
/* Generate initial parameters with yargs, add data, then return node actions. */
const node = (argv, useYargs = true) => {
    const params = useYargs ? computeArgv(argv) : argv;
    params.url = `http://${params.host}:${params.port}`;
    params.provider = new ethers_1.ethers.providers.StaticJsonRpcProvider(params.url);
    const devNode = new devNode_1.default(params.provider);
    return {
        connect() {
...
exports.node = node;
exports.default = exports.node;
/* If running as script, start anvil. */
if (require.main === module) {
    const main = async () => {
        const { spawnEndedPromise } = await (0, exports.node)({
            pipe: true,
        }).connect();
