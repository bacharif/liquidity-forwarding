...
exports._createSigner = exports.getProviderNetwork = exports.Mnemonic = void 0;
const ethers_1 = require("ethers");
const logger_1 = require("./util/logger");
const readJsonWallet_1 = require("./util/readJsonWallet");
class Mnemonic {
    constructor(mnemonic, iterateOn = "index") {
        this.mnemonic = mnemonic;
        this.iterateOn = iterateOn;
    }
    static path(iterator, iterateOn) {
    ...
    signer(iterator) {
        ...
        return ethers_1.ethers.Wallet.fromMnemonic(this.mnemonic, path);
    }
    address(iterator) {
 ...
 *     name and ID.
 */
async function getProviderNetwork(_provider) {
    let networkId;
    if (_provider["send"]) {
    ...
    if (networkId === 31337) {
        ...
        networkName = ethers_1.ethers.providers.getNetwork(networkId).name;
    }
    return {
        id: networkId,
...
class LoggingProvider extends ethers_1.providers.JsonRpcProvider {
 ...
 */
async function _createSigner(options = {}) {
    let readOnly = false;
    if (options.signer && options.signer.provider) {
        logger_1.logger.debug("Uses provider from given signer", {
            contextInfo: "eth.signer",
            data: (0, logger_1.logdataLimiter)({ signer: options.signer }),
        });
    ...
    if (typeof provider === "string") {
        logger_1.logger.debug("Uses given provider", {
            contextInfo: "eth.signer",
        ...
        provider =
            process.env["MGV_NODE_DEBUG"] === "true"
                ...
                : new ethers_1.providers.JsonRpcProvider(provider);
    }
    else if (provider instanceof ethers_1.ethers.providers.JsonRpcProvider) {
        logger_1.logger.debug("Uses given provider", {
            contextInfo: "eth.signer",
            data: { signer: options.signer },
        });
        ...
        logger_1.logger.debug("Uses ethers' Web3Provider created from given provider", {
            contextInfo: "eth.signer",
        ...
        provider = new ethers_1.ethers.providers.Web3Provider(provider);
    }
    if (provider.getSigner &&
        ...
        await signer.getAddress().catch(() => {
            logger_1.logger.warn("Cannot use signer retrieved from provider.getSigner", {
                contextInfo: "eth.signer",
                data: { signer: signer },
            });
            signer = undefined;
    ...
    if (signer &&
        (!!options.privateKey ||
            ...
            !!options.signer ||
            !!options.jsonWallet)) {
        logger_1.logger.warn("Signer info provided will override default signer", {
            contextInfo: "eth.signer",
            data: { signer: signer },
        });
    }
    ...
    if (options.signer) {
        try {
            ...
            console.warn("provided signer object is not able to reinstantiate on new provider info.");
            throw e;
        }
        if (options.mnemonic || options.privateKey) {
            logger_1.logger.warn("options.signer overrides mnemonic and privateKey", {
                contextInfo: "eth.signer",
                data: { signer: options.signer },
            });
        }
        ...
        signer = new ethers_1.ethers.Wallet(options.privateKey, provider);
        if (options.signerIndex) {
            logger_1.logger.warn("options.signerIndex not applicable to private keys", {
                contextInfo: "eth.signer",
            });
        }
        if (options.mnemonic) {
        ...
        signer = new ethers_1.ethers.Wallet(ethers_1.ethers.Wallet.fromMnemonic(options.mnemonic, options.path), provider);
        if (options.signerIndex) {
            logger_1.logger.warn("options.signerIndex not applicable to mnemonic, use options.path instead.", {
                contextInfo: "eth.signer",
            });
        }
    }
    else if (options.jsonWallet) {
        signer = await (0, readJsonWallet_1.readJsonWallet)(options.jsonWallet.path, options.jsonWallet.password, provider);
    }
    else if (!signer) {
        logger_1.logger.warn("No signing info provided or forceReadOnly is true: only read methods will work.", {
            contextInfo: "eth.signer",
        });
        readOnly = true;
        signer = new ethers_1.ethers.VoidSigner("0x0000000000000000000000000000000000000001", provider);
    }
    return { readOnly, signer };
}
exports._createSigner = _createSigner;
