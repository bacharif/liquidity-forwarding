...
exports.aliases = ["env-overview"];
exports.describe = "reports the current environment and warns of any discrepancies";
...
async function handler(argv) {
    ...
    const mangroveConfigurationInfoPromise = getMangroveConfigurationInfo(argv.nodeUrl);
    const dAppEnvironmentInfoPromise = getMangroveJsAppEnvironmentInfo(COMPONENT_DAPP, DAPP_URL);
    const cleanerBotEnvironmentInfoPromise = getMangroveJsAppEnvironmentInfo(COMPONENT_BOT_CLEANER, CLEANING_BOT_URL);
    const noiseMakerBotEnvironmentInfoPromise = getMangroveJsAppEnvironmentInfo(COMPONENT_BOT_MAKER_NOISE, NOISE_MAKER_BOT_URL);
    const greedyTakerBotEnvironmentInfoPromise = getMangroveJsAppEnvironmentInfo(COMPONENT_BOT_TAKER_GREEDY, GREEDY_TAKER_BOT_URL);
    const updateGasBotEnvironmentInfoPromise = getMangroveJsAppEnvironmentInfo(COMPONENT_BOT_UPDATEGAS, UPDATE_GAS_BOT_URL);
    ...
    const noiseMakerBotEnvironmentInfo = await noiseMakerBotEnvironmentInfoPromise;
    const greedyTakerBotEnvironmentInfo = await greedyTakerBotEnvironmentInfoPromise;
    const updateGasBotEnvironmentInfo = await updateGasBotEnvironmentInfoPromise;
    const { notes: crossComponentNotes, warnings: crossComponentWarnings, contractAddressesPerComponent, } = analyzeEnvironment(repoEnvironmentInfo.info, mangroveJsEnvironmentInfo.info, mangroveConfigurationInfo.info, dAppEnvironmentInfo.info, cleanerBotEnvironmentInfo.info, noiseMakerBotEnvironmentInfo.info, greedyTakerBotEnvironmentInfo.info, updateGasBotEnvironmentInfo.info);
    const notes = [
        ...crossComponentNotes,
    ...
    if (argv.jsonOutput) {
        ...
        if (warnings.length > 0) {
            ...
            warnings.forEach((w) => console.warn(`${w.components.join(", ")}: ${w.content}`));
            console.groupEnd();
            console.log();
        }
        if (notes.length > 0) {
            console.group("NOTES");
            notes.forEach((n) => console.log(`${n.components.join(", ")}: ${n.content}`));
            console.groupEnd();
            console.log();
        }
        const addressesTable = {};
        contractAddressesPerComponent.forEach((innerMap, contractName) => (addressesTable[contractName] = Object.fromEntries(innerMap)));
...
function analyzeEnvironment(repoEnvInfo, mangroveJsEnvInfo, mangroveConfInfo, dAppEnvironmentInfo, cleanerBotEnvironmentInfo, noiseMakerBotEnvironmentInfo, greedyTakerBotEnvironmentInfo, updateGasBotEnvironmentInfo) {
    let notes = [];
    let warnings = [];
    const mgvOracleAddress = repoEnvInfo.contractAddresses.get(CONTRACT_MGV_ORACLE);
    if (!mangroveConfInfo.globalConfig.useOracle ||
    ...
    const { notes: addressNotes, warnings: addressWarnings, contractAddressesPerComponent, } = analyzeAddresses(repoEnvInfo, mangroveJsEnvInfo);
    notes = [...notes, ...addressNotes];
    warnings = [...warnings, ...addressWarnings];
    const { notes: dAppNotes, warnings: dAppWarnings } = analyzeMangroveJsApp(COMPONENT_DAPP, dAppEnvironmentInfo, mangroveJsEnvInfo);
    notes = [...notes, ...dAppNotes];
    warnings = [...warnings, ...dAppWarnings];
    const { notes: cleanerBotNotes, warnings: cleanerBotWarnings } = analyzeMangroveJsApp(COMPONENT_BOT_CLEANER, cleanerBotEnvironmentInfo, mangroveJsEnvInfo);
    notes = [...notes, ...cleanerBotNotes];
    warnings = [...warnings, ...cleanerBotWarnings];
    const { notes: noiseMakerBotNotes, warnings: noiseMakerBotWarnings } = analyzeMangroveJsApp(COMPONENT_BOT_MAKER_NOISE, noiseMakerBotEnvironmentInfo, mangroveJsEnvInfo);
    notes = [...notes, ...noiseMakerBotNotes];
    warnings = [...warnings, ...noiseMakerBotWarnings];
    const { notes: greedyTakerBotNotes, warnings: greedyTakerBotWarnings } = analyzeMangroveJsApp(COMPONENT_BOT_TAKER_GREEDY, greedyTakerBotEnvironmentInfo, mangroveJsEnvInfo);
    notes = [...notes, ...greedyTakerBotNotes];
    warnings = [...warnings, ...greedyTakerBotWarnings];
    const { notes: updateGasBotNotes, warnings: updateGasBotWarnings } = analyzeMangroveJsApp(COMPONENT_BOT_UPDATEGAS, updateGasBotEnvironmentInfo, mangroveJsEnvInfo);
...
function analyzeAddresses(repoEnvInfo, mangroveJsEnvInfo) {
    const notes = [];
    const warnings = [];
    const contractAddressesPerComponent = new Map();
    addComponentContractAddresses(COMPONENT_MANGROVE_REPO, repoEnvInfo.contractAddresses, contractAddressesPerComponent);
    addComponentContractAddresses(COMPONENT_MANGROVE_JS, mangroveJsEnvInfo.contractAddresses, contractAddressesPerComponent);
    for (const [contractName, addressesPerComponent,] of contractAddressesPerComponent) {
        const contractAddressInRepo = addressesPerComponent.get(COMPONENT_MANGROVE_REPO);
        for (const component of [COMPONENT_MANGROVE_JS]) {
            const contractAddressInComponent = addressesPerComponent.get(component);
            if (contractAddressInRepo === undefined &&
...
function addComponentContractAddresses(component, contractAddresses, contractAddressesPerComponent) {
    for (const [contractName, address] of contractAddresses) {
        addOrUpdate(contractAddressesPerComponent, contractName, () => new Map([[component, address]]), (map) => map.set(component, address));
    }
}
function addOrUpdate(map, key, createValue, updateValue) {
    if (map.has(key)) {
...
function analyzeMangroveJsApp(component, mangroveJsAppEnvironmentInfo, mangroveJsEnvInfo) {
    const notes = [];
    const warnings = [];
    if (mangroveJsAppEnvironmentInfo.mangroveJsVersion !==
...
async function getRepoEnvironmentInfo() {
    ...
    for (const contractName of CONTRACTS) {
        ...
        const fetchPromise = (0, utils_1.fetchJson)(gitHubUrlForDeploymentJsonFile)
            .then((json) => {
            if (json.address !== undefined) {
                contractAddresses.set(contractName, json.address);
            }
            else {
                console.warn(`Deployment json file for contract '${contractName}' did not contain an address`);
            }
        })
            .catch((e) => {
            console.error(`Error encountered when fetching deployment json file for contract '${contractName}'`, e);
        });
...
async function getMangroveJsEnvironmentInfo() {
    ...
    const npmjsRegistryUrl = "https://registry.npmjs.org/@mangrovedao/mangrove.js";
    await (0, utils_1.fetchJson)({
        ...
        if (json["dist-tags"]?.latest !== undefined) {
            ...
            console.warn(`JSON returned by npmjs registry did not contain dist-tags/latest`, npmjsRegistryUrl);
        }
    })
        .catch((e) => {
        console.error("Error encountered while fetching package info for mangrove.js", e);
    });
    ...
    return {
        ...
        info: {
            ...
            localPackageVersion,
            contractAddresses: new Map(__1.Mangrove.getAllAddresses("maticmum")),
        },
    };
}
const tokens = ["WETH", "DAI", "USDC"];
async function getMangroveConfigurationInfo(nodeUrl) {
    const notes = [];
    const warnings = [];
    const mgv = await __1.Mangrove.connect(nodeUrl).catch((reason) => {
        warnings.push({
...
async function getMangroveJsAppEnvironmentInfo(component, url) {
    const notes = [];
    const warnings = [];
    let mangroveJsVersion;
    const urlForEnvInfoJsonFile = `${url}/environmentInformation.json`;
    await (0, utils_1.fetchJson)(urlForEnvInfoJsonFile)
        .then((json) => {
...
//# sourceMappingURL=parrotCmd.js.map